<template>
  <div class="days-tab text-center">
    ...
    <ul v-else class="p-0 d-flex days">
...
export default {
  ...
  methods: {
    async fetchWeatherData() {
      const apiKey = "5165e738fc14279c38d921614426c7bc";
      ...
      await axios
        ...
        .then((Response) => {
          ...
          const filteredData = forcastData
            .map((item) => {
              return {
            ...
            .reduce((acc, item) => {
              if (!acc.some((day) => day.date.isSame(item.date, "dat"))) {
                acc.push(item);
              }
              return acc;
            }, [])
          ...
          this.loading = false;
          // console.log(forcastData, filteredData);
        })
        .catch((error) => {
